...
module.exports = function validate (testSchema, origValue, output, error, validatorObj) {
  ...
  if (Array.isArray(value) && testSchema.items) {
  ...
  } else if (lodash.isObject(value) && testSchema.properties) {
    output = {};
    lodash.keys(testSchema.properties).forEach(function (key) {
      var rslt = validate(
        testSchema.properties[key],
        value[key],
        {},
        {},
        validatorObj
